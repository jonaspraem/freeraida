...
export class SocialService {
  ...
  public followUser(username: string): Observable<IUserProfile> {
    ...
    return this._http.post<IUserProfile>(this._config.getEndpoint() + '/api/social/follow/' + username, body, {
      headers: headers,
  ...
  public unfollowUser(username: string): Observable<IUserProfile> {
    ...
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    const token = localStorage.getItem('api_token');
    return this._http.post<IUserProfile>(this._config.getEndpoint() + '/api/social/unfollow/' + username, body, {
      headers: headers,
      params: new HttpParams().set('token', token),
    });
  }
  ...
  public getFollowers(username: string): Observable<IUserProfile[]> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    const token = localStorage.getItem('api_token');
    return this._http.get<IUserProfile[]>(this._config.getEndpoint() + '/api/social/followers/' + username, {
      headers: headers,
      params: new HttpParams().set('token', token),
    });
  }
  ...
  public getFollowing(username: string): Observable<IUserProfile[]> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    const token = localStorage.getItem('api_token');
    return this._http.get<IUserProfile[]>(this._config.getEndpoint() + '/api/social/following/' + username, {
      headers: headers,
      params: new HttpParams().set('token', token),
    });
  }
