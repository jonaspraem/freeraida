...
export class LineService {
  ...
  public getLineInfo(line: ILineLocation[]): Observable<any> {
    ...
    return this.http.post(this.config.getEndpoint() + '/api/location-service/', body, {
      headers: headers,
  ...
  public saveLine(line: ILine): Observable<any> {
    ...
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    const token = localStorage.getItem('api_token');
    return this.http.post(this.config.getEndpoint() + '/api/line/new', body, {
      headers: headers,
      params: new HttpParams().set('token', token),
    });
  }
  ...
  public getUserLines(username: string): Observable<ILine[]> {
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    const token = localStorage.getItem('api_token');
    return this.http.get<ILine[]>(this.config.getEndpoint() + '/api/line/user/' + username, {
      headers: headers,
      params: new HttpParams().set('token', token),
    });
  }
  ...
  public updateLine(line: ILine): Observable<ILine> {
    ...
    const headers = new HttpHeaders({ 'Content-Type': 'application/json' });
    const token = localStorage.getItem('api_token');
    return this.http.patch<ILine>(this.config.getEndpoint() + '/api/line/' + line._id, body, {
      headers: headers,
      params: new HttpParams().set('token', token),
    });
  }
